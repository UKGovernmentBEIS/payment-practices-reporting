# --- !Ups
create table "short_form" ("report_id" BIGSERIAL NOT NULL PRIMARY KEY,"company_name" VARCHAR(255) NOT NULL,"company_id" VARCHAR(255) NOT NULL,"filing_date" date NOT NULL,"approved_by" VARCHAR(255) NOT NULL,"confirmation_email_address" VARCHAR(255) NOT NULL,"start_date" date NOT NULL,"end_date" date NOT NULL,"payment_codes" VARCHAR(245));
create table "long_form" ("report_id" BIGINT NOT NULL,"payment_terms" VARCHAR(35000) NOT NULL,"payment_period" INTEGER NOT NULL,"maximum_contract_period" INTEGER NOT NULL,"maximum_contract_period_comment" VARCHAR(3500),"payment_terms_changed_comment" VARCHAR(3500),"payment_terms_changed_notified_comment" VARCHAR(3500),"payment_terms_comment" VARCHAR(3500),"dispute_resolution" VARCHAR(14000) NOT NULL,"offer_einvoicing" BOOLEAN NOT NULL,"offer_supply_chain_finance" BOOLEAN NOT NULL,"retention_charges_in_policy" BOOLEAN NOT NULL,"retention_charges_in_past" BOOLEAN NOT NULL,"average_days_to_pay" INTEGER NOT NULL,"percent_paid_later_than_agreed_terms" INTEGER NOT NULL,"percent_invoices_within30days" INTEGER NOT NULL,"percent_invoices_within60days" INTEGER NOT NULL,"percent_invoices_beyond60days" INTEGER NOT NULL);
create unique index "longForm_report_idx" on "long_form" ("report_id");
...
alter table "long_form" add constraint "longForm_report_fk" foreign key("report_id") references "short_form"("report_id") on update NO ACTION on delete CASCADE;
alter table "confirmation_pending" add constraint "confirmationpending_report_fk" foreign key("report_id") references "short_form"("report_id") on update NO ACTION on delete CASCADE;
alter table "confirmation_sent" add constraint "confirmationsent_report_fk" foreign key("report_id") references "short_form"("report_id") on update NO ACTION on delete CASCADE;
alter table "confirmation_failed" add constraint "confirmationfailed_report_fk" foreign key("report_id") references "short_form"("report_id") on update NO ACTION on delete CASCADE;
